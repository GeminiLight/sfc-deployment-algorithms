...
class PhysicalNetwork(object):
    ...
    def nodes(self):
        ...
        # nodes = [(nodes_attrs[i]['cpu_max'], nodes_attrs[i]['cpu_free'], nodes_attrs[i]['bw_sum_max'], nodes_attrs[i]['bw_sum_free']) for i in range(pn.nodes_num)]
        ...
        # return np.array(np.array(self.raw_nodes_data.iloc[:, 2:6]) , dtype=np.float32)
    ...
    def norm_nodes_data(self):
        ...
        norm_nodes_data = np.concatenate(
            (norm_cpu, norm_ram, norm_rom, norm_bw_sum), axis=-1)
    ...
    def find_candidate_nodes(self, cpu_req=0, ram_req=0, rom_req=0, rtype='id'):
        if rtype == 'id':
            ...
            return np.bitwise_and(cpu_subset, ram_subset, rom_subset)
    ...
    def update_bw_with_path(self, path, x):
        ...
        # self.raw_edges_data.loc[int(eid), attr] -= x
